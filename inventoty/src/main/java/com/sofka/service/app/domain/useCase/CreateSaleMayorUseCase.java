...
@Service("CreateSaleMayorUseCase")
public class CreateSaleMayorUseCase implements ICreateSaleUseCase {

	private static final String TYPEMAYOR = "MAYOR";

	private final IProductoRepository iProductoRepository;

	public CreateSaleMayorUseCase(IProductoRepository iProductoRepository) {
	...
	public Mono<SaleDto> generate(List<ProductDetailDto> products) {
		...
		return iProductoRepository
				.findAllById(products.stream().map(ProductDetailDto::getIdProduct).distinct().toList()).map(p -> {
					...
					return SaleDto.building().total(totalSale).datail(list).tipo(TYPEMAYOR);
